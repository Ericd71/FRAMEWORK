...
void Image::DrawTriangleInterpolated(const Vector3& p0, const Vector3& p1, const Vector3& p2, const Color& c0, const Color& c1, const Color& c2) {
    ...
    for (int i = 0; i < table.size(); i++) {
        if (table[i].maxx > table[i].minx) {
            for (int j = table[i].minx; j < table[i].maxx; j++) {
                Vector3 bCoords = m * Vector3(j, i, 1);
                bCoords.Clamp(0, 1);
                interp_col = bCoords.x * c0 + bCoords.y * c1 + bCoords.z * c2;
                
                Image::SetPixelSafe(j, i, interp_col);
            }
        }
    }
}

//3.3 Mesh with occlusions (Z-buffer)
    ...
    for (int i = 0; i < table.size(); i++) {
        if (table[i].maxx > table[i].minx) {
            for (int j = table[i].minx; j < table[i].maxx; j++) {
                ...
                float z_i = p0.z * bCoords.x + p1.z * bCoords.y + p2.z * bCoords.z;
                if (z_i < zbuffer->GetPixel(j, i)) {
                    SetPixelSafe(j, i, interp_col) ;
...
//3.4 Color the mesh using texture
            ...
            for (int j = table[i].minx; j < table[i].maxx; j++) {
                ...
                if (z_i < zbuffer->GetPixel(j, i)) {
                    ...
                    if (texture == nullptr) {
                        ...
                        float uv_x = (uv0_tex.x * bCoords.x) + (uv1_tex.x * bCoords.y) + (uv2_tex.x * bCoords.z);
                        float uv_y = (uv0_tex.y * bCoords.x) + (uv1_tex.y * bCoords.y) + (uv2_tex.y * bCoords.z);

                        Color texture_color = texture->GetPixelSafe(uv_x, uv_y);
                        SetPixelSafe(j, i, texture_color);
                    }
                }
            }
        }
...
//Drawing triangles for general usage
void Image::DrawTriangleInterpolated(const sTriangleInfo& triangle, FloatImage* zbuffer) {
        std::vector<Cell> table;
        table.resize(height);
        ScanLineDDA(triangle.vertices[0].position.x, triangle.vertices[0].position.y, triangle.vertices[1].position.x, triangle.vertices[1].position.y, table);
        ScanLineDDA(triangle.vertices[1].position.x, triangle.vertices[1].position.y, triangle.vertices[2].position.x, triangle.vertices[2].position.y, table);
        ScanLineDDA(triangle.vertices[0].position.x, triangle.vertices[0].position.y, triangle.vertices[2].position.x, triangle.vertices[2].position.y, table);

        Color interp_col;
        Matrix44 m;
        m.SetIdentity();

        m.M[0][0] = triangle.vertices[0].position.x; m.M[1][0] = triangle.vertices[1].position.x; m.M[2][0] = triangle.vertices[2].position.x;
        m.M[0][1] = triangle.vertices[0].position.y; m.M[1][1] = triangle.vertices[1].position.y; m.M[2][1] = triangle.vertices[2].position.y;
        m.M[0][2] = 1; m.M[1][2] = 1; m.M[2][2] = 1;
        ...
        for (int i = 0; i < table.size(); i++) {
            if (table[i].maxx > table[i].minx) {
                for (int j = table[i].minx; j < table[i].maxx; j++) {
                    ...
                    interp_col = bCoords.x * triangle.vertices[0].color + bCoords.y * triangle.vertices[1].color + bCoords.z * triangle.vertices[0].color;
                    
                    float z_i = triangle.vertices[0].position.z * bCoords.x + triangle.vertices[1].position.z * bCoords.y + triangle.vertices[2].position.z * bCoords.z;
                    if (z_i < zbuffer->GetPixel(j, i)) {
                        SetPixelSafe(j, i, interp_col) ;
                        zbuffer->SetPixel(j, i, z_i);
                        if (triangle.texture == nullptr) {
                            ...
                            Vector2 uv0_tex = Vector2((triangle.vertices[0].uv.x * triangle.texture->width - 1), (triangle.vertices[0].uv.y * triangle.texture->height - 1));
                            Vector2 uv1_tex = Vector2((triangle.vertices[1].uv.x * triangle.texture->width - 1), (triangle.vertices[1].uv.y * triangle.texture->height - 1));
                            Vector2 uv2_tex = Vector2((triangle.vertices[2].uv.x * triangle.texture->width - 1), (triangle.vertices[2].uv.y * triangle.texture->height - 1));

                            float uv_x = (uv0_tex.x * bCoords.x) + (uv1_tex.x * bCoords.y) + (uv2_tex.x * bCoords.z);
                            float uv_y = (uv0_tex.y * bCoords.x) + (uv1_tex.y * bCoords.y) + (uv2_tex.y * bCoords.z);

                            Color texture_color = triangle.texture->GetPixelSafe(uv_x, uv_y);
                            SetPixelSafe(j, i, texture_color);
                        }
                    }
                }
